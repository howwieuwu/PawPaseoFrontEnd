...
export default function Page() {
  ...
  const setActiveWalker = useHistorial((state) => state.setActiveWalker);
  ...
  useEffect(() => {
    axios
      .get("https://pawpaseo-backend-phi.vercel.app/api/paseadores")
      .then((response) => {
        setLoading(false);
        if (Array.isArray(response.data.walkerFound)) {
          ...
          console.error(
            "La respuesta de la API no contiene 'walkerFound' como un array:",
            response.data
          );
        }
      })
  ...
  const AbrirGrafica = (paseador) => {
    setActiveWalker(paseador._id);
    setIsModalGrafica(true);
  };

  const toggleActivo = (id, estadoActual) => {
    const nuevoEstado = !estadoActual;
    axios
      .put(`https://pawpaseo-backend-phi.vercel.app/api/paseador/${id}`, {
        estado: nuevoEstado,
      })
      .then((response) => {
        setPaseadores(
  ...
  const filteredPaseadores = paseadores.filter(
    (paseador) =>
  ...
  const updatePaseador = (id, updatedData) => {
    axios
      .put(
        `https://pawpaseo-backend-phi.vercel.app/api/paseador/${id}`,
        updatedData
      )
      .then((response) => {
        setPaseadores(
          paseadores.map((paseador) =>
            paseador._id === id ? { ...paseador, ...updatedData } : paseador
          )
        );
        setIsModalOpen(false);
              ...
              <tr key={paseador._id} className="h-14 border-y-2">
                ...
                <td className="flex justify-center">
                  <BiShow
                    ...
                    onClick={() => handleView(paseador)}
                  />
                  <TbEdit
                    className="h-8 w-8 cursor-pointer ml-2"
                    onClick={() => handleEdit(paseador)}
                  />
                  <GoGraph
                    className="h-8 w-8 cursor-pointer ml-2"
                    onClick={() => AbrirGrafica(paseador)}
                  />
                </td>
              </tr>
            ))}
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input
                  type="text"
                  name="nombre"
                  defaultValue={paseadorSeleccionado.nombre}
              ...
              <div className="mb-4">
                ...
                <input
                  type="tel"
                  name="telefono"
                  defaultValue={paseadorSeleccionado.telefono}
                  className="p-2 border rounded w-full"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  type="text"
                  name="ciudad"
                  defaultValue={paseadorSeleccionado.ciudad}
                  className="p-2 border rounded w-full"
                />
              </div>
              <div className="mb-4">
                ...
                <input
                  type="email"
                  name="email"
                  defaultValue={paseadorSeleccionado.email}
                  className="p-2 border rounded w-full"
                />
              </div>
          ...
          <div className="bg-white p-8 rounded-lg">
            ...
            <form>
              <img
                ...
                src={paseadorVer.foto_perfil}
                alt={paseadorVer.nombre}
              />
              <text className="flex items-center justify-center">
                {getIconByCalificacion(paseadorVer.calificacion)}
                <span className="ml-1">{paseadorVer.calificacion}</span>
              </text>
              <div className="mb-4">
                <label className="block mb-2">Nombre:</label>
                <input
              ...
              <div className="flex justify-center">
                <button
                  type="button"
                  onClick={() => setIsModalOpenVer(false)}
                  className="mr-4 py-2 px-4 bg-[#124C5F] text-white rounded"
                >
                  Cerrar
                </button>
              </div>
